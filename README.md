...
本仓库提供论文 **《FM-VMamba：面向电子喉镜喉部病变分类的频域重加权视觉状态空间模型》** 的模型实现、数据质量控制脚本、辅助实验代码和五折交叉验证结果。

> 本项目仅用于科研与教学，不可直接用于临床诊断或替代专业医生判断。
...
离线数据增广是独立的数据准备步骤，不嵌入 `train.py`。正式复现实验时，应确保增广样本仅属于其对应训练折，不能把不同折产生的增广图像合并后再次随机划分，否则可能造成数据泄漏。

> 当前仓库中的 `offline_augment.py` 为独立的数据增广与固定划分辅助脚本，可用于预处理实验。论文正式五折结果遵循“先划分五折，再仅增广当前训练折”的协议。复现论文结果时，应根据正式折划分清单按折组织训练集和验证集。
...
不同 PyTorch、CUDA 和 VMamba 源码版本可能导致结果存在轻微差异，建议在复现实验时记录完整的软件版本、GPU 型号和随机种子。
...
无论采用目录还是清单形式，都必须保证同一原始图像及其增广版本只出现在同一训练折中。
...
该脚本为独立辅助脚本。用于论文正式五折实验时，应先固定折划分，再仅对当前训练折执行相同增广操作，验证集不得增广。
...
2. 原始数据未保留完整患者级标识，因此不能完全排除同一患者图像跨折分布的风险；
3. 复现实验时应固定随机种子和折划分；
4. 训练折可增广，验证折不得增广；
5. 不得将同一原图及其增广版本分配到不同折；
6. 表 1 主对比实验与表 3 消融实验为独立训练流程，数值不应直接混用；
7. GitHub 中的示例结果用于展示代码输出格式，实际结果可能因环境、权重和数据划分而略有差异。
...
本仓库中的本地路径仅用于记录原始实验环境，不参与其他用户运行。复现时请改为本机路径或相对路径。

## 15. 引用

论文正式发表后，请按照期刊信息引用本文。当前可暂写为：

```bibtex
@article{cui_fmvmamba,
  title   = {FM-VMamba: A Frequency-Reweighted Visual State Space Model for Laryngeal Lesion Classification in Electronic Laryngoscopy},
  author  = {Cui, Qijia},
...
同时请引用 VMamba 和 Mona 的原始工作。

## 16. 许可证

本仓库代码仅供学术研究使用。第三方代码、模型结构和预训练权重仍遵循其原始许可证。正式公开前，建议根据实际使用的第三方组件补充合适的 `LICENSE` 文件。

## 17. 联系方式

如对代码实现或实验复现有疑问，可通过 GitHub Issues 反馈。
